 ...
 */
export async function parsePublicationsCSV(csvData: string): Promise<Publication[]> {
  return new Promise((resolve, reject) => {
    Papa.parse(csvData, {
      header: true,
      complete: (results) => {
        const publications = results.data.map((row: any, index: number) => {
          ...
          return {
            ...
            organisms: row.organisms ? row.organisms.split(';').map((o: string) => o.trim()) : [],
            experimentType: row.experiment_type ? row.experiment_type.split(';').map((e: string) => e.trim()) : [],
            mission: row.mission,
            platform: row.platform
          };
        });
        resolve(publications as Publication[]);
      },
      error: (error) => {
        reject(error);
      }
...
function extractTopics(abstract: string, keywords: string): string[] {
  ...
  const commonTopics = [
    'microgravity', 'radiation', 'space flight', 'bone loss', 'muscle atrophy',
    'cardiovascular', 'immune system', 'plants', 'microbiome', 'neuroscience',
    'genetics', 'cell biology', 'physiology', 'behavior', 'development'
  ];
 ...
 */
export function createSearchIndex(publications: Publication[]): Map<string, number[]> {
  const index = new Map<string, number[]>();
  
  publications.forEach((pub, idx) => {
    // Index title words
    const titleWords = pub.title.toLowerCase().split(/\W+/).filter(w => w.length > 2);
    // Index abstract words
    const abstractWords = pub.abstract.toLowerCase().split(/\W+/).filter(w => w.length > 2);
    // Index keywords
    ...
    const terms = [...new Set([...titleWords, ...abstractWords, ...keywords, ...authors, ...pub.topics])];
 ...
 */
export function getPublicationSummaries(publications: Publication[]): PublicationSummary[] {
  return publications.map(pub => ({
    id: pub.id,
    title: pub.title,
    authors: pub.authors,
    year: pub.year,
    abstract: pub.abstract.substring(0, 200) + (pub.abstract.length > 200 ? '...' : ''),
    topics: pub.topics
  }));
}
 ...
 */
export function calculateTopicDistribution(publications: Publication[]): TopicDistribution[] {
  const topicCounts = new Map<string, { count: number; publications: string[] }>();
 ...
 */
export function calculateYearlyStats(publications: Publication[]): YearlyStats[] {
  const yearMap = new Map<number, { count: number; topics: Map<string, number> }>();
  
  publications.forEach(pub => {
    if (!yearMap.has(pub.year)) {
      yearMap.set(pub.year, { count: 0, topics: new Map() });
    }
 ...
 */
export function identifyResearchGaps(publications: Publication[]): ResearchGap[] {
  ...
  const gaps: ResearchGap[] = [
    ...
    ...Array.from(decliningTopics).map(topic => {
      const topicData = topicDistribution.find(t => t.topic === topic);
      return {
        topic,
        severity: 0.7,
        relatedTopics: findRelatedTopics(topic, publications),
 ...
 */
function findRelatedTopics(topic: string, publications: Publication[]): string[] {
  const coOccurrence = new Map<string, number>();
  
  publications.forEach(pub => {
    if (pub.topics.includes(topic)) {
