...
import { Publication, YearlyStats, TopicDistribution as TopicDistributionType, ResearchGap } from '../../types/publication';
import { parsePublicationsCSV, calculateYearlyStats, calculateTopicDistribution, identifyResearchGaps } from '../../lib/dataProcessor';
...
export default function VisualizationsPage() {
  ...
  const [topicDistribution, setTopicDistribution] = useState<TopicDistributionType[]>([]);
  ...
  const handleTopicClick = (topic: string) => {
    ...
    const relatedPubs = publications.filter(pub => pub.topics.includes(topic));
      ...
      <div className="bg-slate-900 py-8">
        <div className="container mx-auto px-4">
          ...
          <p className="text-gray-400 mb-6">
            Interactive visualizations of NASA space biology research publications data.
          </p>
          
          {/* Visualization nav */}
          <div className="flex flex-wrap gap-4">
            <a href="#timeline" className="flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-500/20 hover:bg-blue-500/30 transition-colors">
              <BarChart2 size={18} />
              <span>Timeline</span>
            </a>
            <a href="#topics" className="flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-500/20 hover:bg-blue-500/30 transition-colors">
              <PieChart size={18} />
              <span>Topic Distribution</span>
            </a>
            <a href="#gaps" className="flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-500/20 hover:bg-blue-500/30 transition-colors">
              <TrendingDown size={18} />
              <span>Research Gaps</span>
            </a>
            <a href="#network" className="flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-500/20 hover:bg-blue-500/30 transition-colors">
              <Network size={18} />
          ...
          <div className="space-y-12">
              ...
              <div className="mt-6 glass-card p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <p className="text-gray-300">
                      {yearlyStats.length > 0 ? (
                        `Publication output has ${
                          yearlyStats[yearlyStats.length - 1].publicationCount > yearlyStats[0].publicationCount
                            ? 'increased'
                            : 'decreased'
                        } over time, with the highest number of publications (${
                          Math.max(...yearlyStats.map(stat => stat.publicationCount))
                        }) in ${
                          yearlyStats.find(stat => 
                            stat.publicationCount === Math.max(...yearlyStats.map(s => s.publicationCount))
                          )?.year
                        }.`
                  ...
                  <div>
                    ...
                    <p className="text-gray-300">
                      The focus of research has evolved over time, with recent years showing increased interest in
                      {yearlyStats.length > 0 && yearlyStats[yearlyStats.length - 1].topTopics.length > 0 ? (
                        ` ${yearlyStats[yearlyStats.length - 1].topTopics.slice(0, 3).map(t => t.topic).join(', ')}.`
            ...
            <section id="topics">
              ...
              <div className="mt-6 glass-card p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <h4 className="font-medium mb-2">Dominant Research Areas</h4>
                    <p className="text-gray-300">
                      {topicDistribution.length > 0 ? (
                        `The most researched topics are ${
                          topicDistribution.slice(0, 3).map(t => t.topic).join(', ')
                        }, accounting for ${
                          Math.round((topicDistribution.slice(0, 3).reduce((sum, t) => sum + t.count, 0) / 
                          topicDistribution.reduce((sum, t) => sum + t.count, 0)) * 100)
                        }% of all publications.`
                  ...
                  <div>
                    ...
                    <p className="text-gray-300">
                      {topicDistribution.length > 0 ? (
                        `Research spans ${topicDistribution.length} distinct topics, with ${
                          topicDistribution.filter(t => t.count === 1).length
            ...
            <section id="gaps">
              ...
              <div className="mt-6 glass-card p-6">
                <h3 className="text-lg font-bold mb-4">Gap Analysis Insights</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <h4 className="font-medium mb-2">Critical Research Needs</h4>
                    <p className="text-gray-300">
                      {researchGaps.length > 0 ? (
                        `The most significant research gaps are in ${
                          researchGaps.slice(0, 2).map(g => g.topic).join(' and ')
                  ...
                  <div>
                    ...
                    <p className="text-gray-300">
                      These identified gaps represent potential funding opportunities and areas where new research could have significant impact on space biology knowledge.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 glass-card p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="text-gray-300">
                      Publications form distinct research clusters around key topics, with some publications bridging multiple research areas.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-gray-300">
                      The network reveals collaboration patterns between researchers and institutions, highlighting the interdisciplinary nature of space biology research.
                    </p>
                  </div>
                </div>
              </div>
