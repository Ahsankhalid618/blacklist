...
export async function GET() {
  try {
    const originalPath = path.join(
      process.cwd(),
      "data",
      "PMC_Articles_Data.xlsx"
    );
    const backupPath = path.join(
      process.cwd(),
      "data",
      "PMC_Articles_Data_copy.xlsx"
    );
    ...
    interface ExcelRow {
      ...
      Abstract?: string;
      Citation?: string;
      "Full Text Available"?: string;
      "Scraping Success"?: string;
      Error?: string;
      Section_1_Name?: string;
      Section_1_Content?: string;
      Section_2_Name?: string;
      Section_2_Content?: string;
      Section_3_Name?: string;
      Section_3_Content?: string;
    }

    const publications: Publication[] = data.map((row: ExcelRow) => {
